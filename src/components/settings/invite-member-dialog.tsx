...
export function InviteMemberDialog({
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setResult({
        ...
        accountName: account?.name ?? 'our TRAFIKOS account',
      });
      onCreated();
    } catch (err) {
  ...
  function whatsappShareUrl(url: string): string {
    ...
    // for users in multi-team contexts where "our TRAFIKOS account"
    // wouldn't be enough to disambiguate.
    const accountName = result?.accountName ?? 'our TRAFIKOS account';
    const message = `Join ${accountName} on TRAFIKOS using this link (valid for ${result?.expiresInDays} days): ${url}`;
