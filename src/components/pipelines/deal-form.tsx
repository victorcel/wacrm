...
export function DealForm({
  ...
  async function handleSave() {
    if (!title.trim() || !contactId || !stageId) {
      toast.error("El título, el contacto y la etapa son obligatorios");
      return;
    }
    setSaving(true);
    ...
    if (deal) {
      const { error } = await supabase
        ...
        .eq("id", deal.id);
      if (error) {
        toast.error("No se pudo guardar el negocio");
        setSaving(false);
        return;
      }
    } else {
      ...
      >
        <div className="flex h-full flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            <div className="grid gap-2">
              <Label className="text-muted-foreground">Fecha de cierre prevista</Label>
              <Input
                type="date"
            ...
            {deal &&
              (confirmDelete ? (
                <div className="mt-3 flex items-center justify-between gap-2 rounded-md border border-red-500/30 bg-red-500/10 px-3 py-2 text-xs">
                  <span className="text-red-300">¿Eliminar este negocio?</span>
