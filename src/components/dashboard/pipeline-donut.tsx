    ...
    <section className="flex h-full flex-col rounded-xl border border-border bg-card">
      ...
      <div className="flex flex-1 flex-col p-5">
        {loading || !data ? (
          ...
          <EmptyState
            icon={GitBranch}
            title="Aún no hay negocios abiertos"
            hint="Crea negocios en Embudos para ver el desglose por etapa aquí."
          />
        ) : (
          <>
...
function Donut({ data, currency }: { data: PipelineDonutData; currency: string }) {
  ...
  return (
    <div className="flex items-center justify-center">
      <svg viewBox={`0 0 ${size} ${size}`} className="h-48 w-48" role="img" aria-label="Valor del embudo por etapa">
        {/* background ring */}
