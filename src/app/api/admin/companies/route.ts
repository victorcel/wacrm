...
export async function GET() {
  try {
    ...
    const [accountsRes, subsRes, profilesRes] = await Promise.all([
      admin
        .from("accounts")
      ...
      admin.from("profiles").select("account_id, email, account_role"),
    ]);
    ...
    for (const p of profilesRes.data ?? []) {
      ...
      if (p.account_role === "owner" && p.email) ownerEmail.set(p.account_id, p.email);
    }
